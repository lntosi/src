...
Interest::Interest(const Name& name, time::milliseconds lifetime) : m_interestLifetime(DEFAULT_INTEREST_LIFETIME)
{
  setName(name);
...
Interest::Interest(const Block& wire) : m_interestLifetime(DEFAULT_INTEREST_LIFETIME)
{
  wireDecode(wire);
}
...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // MPRList
  if (!getMPRList().empty()) {
    totalLength += getMPRList().wireEncode(encoder);
  }
...
Interest::wireDecode(const Block& wire)
{
  ...
  for (++element; element != m_wire.elements_end(); ++element) {
    switch (element->type()) {
      ...
      case tlv::MPRList: {
        if (lastElement >= 5) {
          NDN_THROW(Error("MPRList element is out of order"));
        }
        m_mprList.wireDecode(*element);
        lastElement = 5;
        break;
      }
      case tlv::Nonce: {
...
Interest& Interest::setMPRList(const MPRList& lst)
{
  m_mprList = lst;
  return *this;
}

const MPRList& Interest::getMPRList() const noexcept
{
  return m_mprList;
}
